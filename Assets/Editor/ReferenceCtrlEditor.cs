...
#if UNITY_EDITOR
//自定义ReferenceCtrl类在界面中的显示与功能
[CustomEditor(typeof(ReferenceCtrl))]
//没有该属性的编辑器在选中多个物体时会提示“Multi-object editing not supported”
[CanEditMultipleObjects]
public class ReferenceCtrlEditor : Editor
{
    static Type[] sAllComponentTypes =
    {
        typeof(CanvasGroup),
        typeof(ReferenceCtrl),
    ...
    static string UI_CODE_PATH = "/Scripts/UI/";
    ...
    string searchKey
    {
        ...
        set
        {
            if (this._searchKey != value)
            {
                ...
                this.heroPrefab = this.ReferenceCtrl.Get<Object>(this.searchKey);
            }
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("刷新UUID"))
        {
            ...
            {
                this.ReferenceCtrl.RefreshUUID();
            }
        }
        string pp = this.ReferenceCtrl.UUID.Substring(0, 8);
        string buttonTitle = $"复制UI_AUTOCODE_RC ({pp})";
        if (GUILayout.Button(buttonTitle))
        {
            string buf =
                "#region UI_AUTOCODE_RC "
                + this.ReferenceCtrl.UUID.Replace("-", "")
                + "\n#endregion";
        ...
        Undo.RecordObject(this.ReferenceCtrl, "Changed Settings");
        ...
        {
            this.ReferenceCtrl.Sort();
        }

        if (GUILayout.Button("同步代码"))
        {
            SyncUICode(this.ReferenceCtrl);
        }

        if (GUILayout.Button("跳转代码"))
        {
            JumpToCode(this.ReferenceCtrl);
        }

        if (GUILayout.Button("同步commonSingleItem代码"))
        {
            SyncUICode(this.ReferenceCtrl, true);
        }
        ...
        {
            this.ReferenceCtrl.Remove(this.searchKey);
        ...
        for (int i = this.ReferenceCtrl.data.Count - 1; i >= 0; i--)
        {
            GUILayout.BeginHorizontal();
    ...
    public static void SyncUICode(ReferenceCtrl rc, bool genCacheByRC = false)
    {
        string path;
        if (SyncUICodeInDir(rc, UI_CODE_PATH, out path, genCacheByRC)) //读取的根路径
        {
    ...
    public static void JumpToCode(ReferenceCtrl rc, string path = null)
    {
        int line = 0;
        if (FindUICodeInDir(rc, UI_CODE_PATH, ref path, out line))
        {
            MonoScript code = AssetDatabase.LoadAssetAtPath<MonoScript>(path);
            if (code != null)
            {
    ...
    public static void NoCodeFoundTip(ReferenceCtrl rc)
    {
    ...
    {
        Object obj = this.ReferenceCtrl.data[dataIndex].gameObject;
    ...
    void FindAll()
    {
        while (this._objects.Count > 0)
        {
            ...
            if (t != null && this.ReferenceCtrl.Get<Object>(t.name) == null)
            {
            ...
            for (int i = 0, count = obj.transform.childCount; i < count; i++)
            {
                ...
                if (child.GetComponent<ReferenceCtrl>() == null)
                {
    ...
    static void AddGetReference(ReferenceCtrl rc, StringBuilder strBuilder, string indent)
    {
        for (int i = 0; i < rc.data.Count; ++i)
        {
            ...
            strBuilder.Append(" = ");
            strBuilder.Append("rc.GetReference<" + typeName + ">(" + Convert.ToString(i) + ")");
            strBuilder.Append(";");
    ...
    static string GenRCAutoCode(ReferenceCtrl rc, bool genCacheByRC)
    {
        ...
        for (int i = 0; i < rc.data.Count; ++i)
        {
            ...
            strBuilder.Append(indent);
            strBuilder.Append($"private {typeName} {data.key};\n");
        }
        ...
        strBuilder.Append("{\n");
        strBuilder.Append(indent);
        strBuilder.Append("    var rc = this.gameObject.GetComponent<ReferenceCtrl>();\n");
        ...
        if (genCacheByRC)
        {
            strBuilder.Append("\n");
            strBuilder.Append(indent);
            strBuilder.Append("public void CacheReference(ReferenceCtrl rc)\n");
            strBuilder.Append(indent);
            strBuilder.Append("{\n");
            strBuilder.Append(indent);
    ...
    void OnEnable()
    {
        //将被选中的gameobject所挂载的ReferenceCtrl赋值给编辑器类中的ReferenceCtrl，方便操作
        this.ReferenceCtrl = (ReferenceCtrl)this.target;
    }

    static bool FindUICodeInDir(ReferenceCtrl rc, string uiRootDir, ref string path, out int line)
    {
        var uuid = $"{rc.UUID}".Replace("-", "");
